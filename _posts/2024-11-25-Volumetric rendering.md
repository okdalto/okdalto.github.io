...
그러나 폴리곤 모델링은 구름, 연기, 불꽃과 같은 볼륨 형태의 물체를 표현하는 데 한계가 있다. 
이러한 복잡한 형태와 밀도 분포를 정확히 표현하기 위해 Volumetric rendering이 필요하다.

## 왜 폴리곤 렌더링에 한계가 있는가? ##

폴리곤 렌더링은 3D 그래픽에서 물체의 표면을 다각형(주로 삼각형)으로 분할하여 표현하는 방식이다. 
이 방식은 물체의 형태를 정의하는 데 효과적이지만, 다음과 같은 이유로 볼륨 형태의 물체를 표현하는 데는 부적합하다.

1. 표면 중심의 표현 방식
폴리곤 렌더링은 주로 물체의 외곽 경계를 정의하며, 내부의 밀도나 상태 변화는 표현할 수 없다. 
예를 들어, 구름이나 연기처럼 내부가 균일하지 않은 구조물은 폴리곤으로 적절히 표현하기 어렵다.

2. 복잡한 형태의 비효율성
구름이나 연기처럼 복잡한 형태를 폴리곤으로 표현하려면 수많은 폴리곤을 생성해야 하며, 이는 계산 비용과 메모리 사용량을 급격히 증가시킨다. 
이로 인해 사실적인 표현이 비효율적이고 제한적이다.

3. 빛의 상호작용 표현의 한계
폴리곤 렌더링은 표면과 빛의 상호작용을 기반으로 동작한다. 
그러나 볼륨 형태의 물체는 빛이 내부를 통과하며 흡수, 산란, 굴절되는 복잡한 상호작용을 수반한다. 
이러한 과정을 폴리곤으로 시뮬레이션하기에는 근본적인 제약이 따른다.

## 볼륨 렌더링이란? ##

물체를 렌더하기 위해서는 먼저 물체의 형태를 어떻게 표현할 것인지 그 표현 방법에 대해서 고려해야만 한다. 
...
이 식은 광선이 $t_near$에서 $t_far$까지 통과하는 동안의 색상을 계산한다. 이를 통해 물체 내부의 색상과 밝기를 계산할 수 있다.
...
사족을 달자면 NeRF 논문을 들여다보면 동일한 식이 등장하는 것을 확인할 수 있다. 
...
Beer's law에 따르면, 빛의 투과도(Transmittance)는 다음과 같이 정의된다.

$$T = e^{-\sigma_a \cdot l}$$

여기서 $T$는 투과도, $\sigma_a$는 흡수계수(Aabsorption coefficient), $l$은 광선이 통과한 거리를 나타낸다. 이 식은 빛이 매질을 통과하면서 감소하는 정도를 설명한다. 즉, 빛이 물체를 통과하면서 그 에너지가 흡수되는 정도를 나타낸다.
...
화면 안으로 Ray를 전진시키기 위해, 각각의 Ray가 향할 Direction을 정의해 보자

우리는 이미 UV좌표를 Attribute로 받고 있기 때문에, 0에서 1 사이의 UV 좌표값을 가지고 있다. 이것을 -1에서 1 사이의 범위로 바꾸고, 이를 이용해서 rayDir을 정의할 것이다.
